...
{
    // Class to generate puzzles with multiple threads
    struct PuzzleGenerator
    {
        struct Params
        {
            // Min and max depths for search during generation
    ...
    void PuzzleGenerator::generate(uint64_t limit)
    {
        ...
        if (limit % params.report_stats_every != 0)
        {
            report(limit, draw_counter, limit % params.report_stats_every);
        }

        std::cout << std::endl;
    }

    void PuzzleGenerator::generate_worker(
        Thread& th,
        std::atomic<uint64_t>& counter,
        std::atomic<uint64_t>& draw_counter,
        uint64_t limit)
    ...
    bool PuzzleGenerator::was_seen_before(const Position& pos)
    {
    ...
    optional<int8_t> PuzzleGenerator::get_current_game_result(
        Position& pos,
        const vector<int>& move_hist_scores) const
    {
    ...
    vector<uint8_t> PuzzleGenerator::generate_random_move_flags(PRNG& prng)
    {
    ...
    optional<Move> PuzzleGenerator::choose_random_move(
        PRNG& prng,
        Position& pos,
        std::vector<uint8_t>& random_move_flag,
        int ply,
    ...
    bool PuzzleGenerator::commit_psv(
        Thread& th,
        PSVector& sfens,
        int8_t result,
        std::atomic<uint64_t>& counter,
    ...
    void PuzzleGenerator::maybe_report(uint64_t done, uint64_t draws)
    {
        if (done % std::min(REPORT_DOT_EVERY, params.report_stats_every) == 0)
        {
            ...
            if (done != 0)
            {
                ...
                if (done % params.report_stats_every == 0)
                {
                    report(done, draws, params.report_stats_every);
                }
            }
        }
    }
    ...
    void generate_puzzles(istringstream& is)
    {
        ...
        while (true)
        {
            ...
            else if (token == "report_stats_every")
            {
                is >> params.report_stats_every;
                const auto DOT = PuzzleGenerator::REPORT_DOT_EVERY;
                if (params.report_stats_every > DOT)
                    params.report_stats_every = params.report_stats_every / DOT * DOT;
                assert(params.report_stats_every > DOT || params.report_stats_every % DOT == 0);
            }
            else if (token == "book")
                is >> params.book;
        ...
        std::cout << "INFO: Executing generate_puzzles command\n";
        ...
        PuzzleGenerator genpuzzle(params);
        genpuzzle.generate(loop_max);

        std::cout << "INFO: generate_puzzles finished." << endl;
    }
}
